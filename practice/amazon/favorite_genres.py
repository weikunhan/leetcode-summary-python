...
and a list of all the songs within that genre as values. 
The song can only belong to only one genre.

The task is to return a map Map<String, List<String>>, where the key is a user 
name and the value is a list of the user's favorite genre(s). 
Favorite genre is the most listened to genre. A user can have more than one 
favorite genre if he/she has listened to the same number of songs per each of 
the genres.

Author: Weikun Han <weikunhan@g.ucla.edu>

Reference: https://leetcode.com/discuss/interview-question/373006
...
class Solution(object):
    def favorite_genres(self, userSongs, songGenres):
        ...
        for user, song_value_list in userSongs.items():
            value_dict = collections.Counter()
            temp_list = []
            
            for song in song_value_list:
                for genre, mapping_value_list in songGenres.items():
                    if song in mapping_value_list:
                        value_dict[genre] += 1
...
def main():
    ...
    songGenres = {  
        ...
        "Jazz":    ["song8", "song9"]}
    solution = Solution()
    res = solution.favorite_genres(userSongs, songGenres)
    print(res)
